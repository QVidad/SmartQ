...
class ExamController extends Controller
{
    ...
    public function indexz(){
        ...
        $user = Auth::user()->id;
        // $top = Topic::all();
        // $top = Examinations::where('exam_type', 1)->get();
        // $top = Tes::with('topic')
        ...
        $top = Examinations::where('start_date', '<=', $now)
                            ->where('end_date', '>=', $now)
                            ->where('exam_type', 1)
                            ->whereHas('studentExams', function ($query) use ($user) {
                            ...
                            ->map(function ($exam) use ($user) {
                                $exam->student_responded = $exam->responses->contains('student_id', $user);
                                return $exam;
                            });
        // dd($top->toArray());
        return view('/exam', compact('user','top'));
     ...
     public function indexTrial() {
        $user = Auth::user()->id;
        // $top = Topic::all();
        $top = Topic::whereHas('subtopics.questions')->get();
     ...
     public function indexResults(){
        ...
        // $totalScore = Tes::where('exam_type', 2)
        //     ->where('student_id', $user)
        //     ->sum('score');

        // $totalItemNum = Tes::where('exam_type', 2)
        //     ->where('student_id', $user)
        //     ->sum('itemnum');
